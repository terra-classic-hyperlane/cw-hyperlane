...
# ═════════════════════════════════════════════════════════════════════════════
# STEP 9 — REGISTRY YAML (Warp UI route file)
#   Generates warp/registry-<token>-config.yaml in the hyperlane-registry
...
if command -v python3 &>/dev/null; then
    if python3 - "$CONFIG_FILE" "$SCRIPT_DIR/warp-sealevel-config.json" "$TOKEN_KEY" "$REG_YAML" <<'REGPY'
import json, sys
...
L.append(f"# Generated by create-warp-evm.sh — registry route file for {t['symbol']}")
